  ...
  return (
    <>
      {friends.map(({ category_name, friends }) => (
        <div
          key={category_name}
          className="mx-1 border-2 dark:border border-gray-700 dark:border-white/10 pt-10 pb-4 rounded-lg bg-white dark:bg-[#1f1f24] dark:shadow-2xl relative mb-5 transition-all"
        >
          ...
          <div className="mx-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
            {friends.map((friend) => (
              <div
                key={friend.name}
                className="flex flex-row gap-5 justify-center autobg items-center py-8 rounded-lg border dark:border border-gray-700 dark:border-white/10 group relative"
              >
